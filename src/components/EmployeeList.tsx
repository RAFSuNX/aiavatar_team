    ...
    <div className="min-h-screen bg-black text-white p-4 sm:p-6 md:p-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="max-w-md mx-auto mb-12 md:mb-16 px-4">
          <div className="p-[1px] bg-gradient-to-r from-[#E70B3B] via-white to-[#E70B3B] rounded-lg overflow-hidden">
            <input
              type="text"
              placeholder="Search by name or role..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full px-6 py-4 bg-gray-900 border-0 focus:ring-0 text-white placeholder-gray-400 text-sm sm:text-base transition-all duration-300 rounded-lg"
            />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 md:gap-8 px-4">
          {filteredEmployees.map((employee, index) => (
            <Link
              key={employee.id}
              to={`/${employee.slug}`}
              className="group p-[1px] bg-gradient-to-r from-[#E70B3B] via-white to-[#E70B3B] rounded-lg shadow-lg hover:shadow-xl hover:scale-[1.02] transition-all duration-300"
              style={{ 
                opacity: 0,
            ...
            >
              <div className="relative w-full h-[220px] overflow-hidden bg-gray-900 rounded-lg">
                <div className="absolute inset-0 w-full h-full">
                  ...
                  />
                  <div className="absolute inset-0 bg-gradient-to-t from-black via-black/50 to-transparent opacity-90 group-hover:opacity-95 transition-all duration-500">
        ...
        {filteredEmployees.length === 0 && (
          <div className="text-center mt-8 p-8 border border-[rgb(var(--color-border))] rounded-lg mx-4 bg-gray-900 shadow-sm">
            <p className="text-lg text-white">No employees found matching your search.</p>
